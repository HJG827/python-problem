        ...
        for c in range(N):
            ...
            for distance in range(1, M):
                for direction in range(4):
                    next_r = r + cross_dr[direction] * distance
                    next_c = c + cross_dc[direction] * distance
                    if 0 <= next_r < N and 0 <= next_c < N:
                        cross_flies += arr[next_r][next_c]
            max_cross = max(cross_flies, max_cross)

            diag_flies = arr[r][c]
            for distance in range(1, M):
                for direction in range(4):
                    next_r = r + diag_dr[direction] * distance
                    next_c = c + diag_dc[direction] * distance
                    if 0 <= next_r < N and 0 <= next_c < N:
                        diag_flies += arr[next_r][next_c]
            max_diag = max(diag_flies, max_diag)
...
for tc in range(1, T+1):
    ...
    print(f'#{tc} {catch_flies(n,m,arrs)}')