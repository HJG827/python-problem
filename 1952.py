...
for tc in range(1, T+1):
    ...
    for i in range(12):
        if use[i]:
            if fee[1] > use[i]*fee[0]:
                result.append(use[i]*fee[0])
            else:
                result.append(fee[1])
        else:
            result.append(0)
    # print('before pop: ', result)

    # while 문으로 바꿔야할듯 중복으로 돌아감!!!
    i = 0
    while i < 10:
        if result[i]+result[i+1]+result[i+2] > fee[2]:
            result[i], result[i+1], result[i+2] = 0, 0, 0
            result.append(fee[2])
            i += 3
        else:
            i += 1
    ...
    #     if result[j] + result[j+1] + result[j+2] > fee[2]:
    #         result[j] , result[j+1] = 0, 0 
    #         result[j+2] = fee[2]
